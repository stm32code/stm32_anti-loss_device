...
mySta Update_oled_massage()
{
	...
	if(device_state_init.close_link){
		sprintf(str, " 连接正常: %02d  ",device_state_init.close_link);
	}else{
		sprintf(str, " 连接断开: %02d  ",device_state_init.close_link);
	}
	...
	sprintf(str, "经度: %06f ",longitude_sum );
	OLED_ShowCH(0, 4, (unsigned char *)str);
	sprintf(str, "纬度: %06f ",latitude_sum);
