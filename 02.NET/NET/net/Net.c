...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"lat\":\"%.6f\",", latitude_sum); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"log\":\"%.6f\",", longitude_sum);
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"heart\":\"%d\",",  device_state_init.close_link); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"waning\":\"%d\",",  device_state_init.waning); 
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"open\":\"%d\"",  device_state_init.open); 
	strcat(buf, text);
...
void SendMqtt(U8 Cmd)
{
	...
	{
		// 封包
		ESP8266_SendMqtt((unsigned char *)buf,body_len);
	}
}
//==========================================================
